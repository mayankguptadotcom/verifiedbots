...
| netcraft-crawler | Netcraft | deferred | — | Operator is clearly identifiable and the web-server survey is legitimate, but no Netcraft-owned crawler documentation could be found: netcraft.com/robots.txt (200) is only `User-agent: * / Allow: /`, and the sitemap (200) lists no bot/FAQ/crawler-policy page (`/faq/` and `/uptime/` both 404). The UA strings, AS15511, and the `195.12.48.0/20` ranges circulating online come from third-party bot directories only — not sourced, not recorded. netcraft.com/resources/web-server-survey is the remaining page to check for a methodology statement. | 2026-07-18 |
| vercel-crawler | Vercel | deferred | — | No operator documentation page exists: vercel.com/docs/bot-management (200) is about managing *other people's* bots and never mentions Vercel's own crawler. The only operator-adjacent source is a 2020 GitHub discussion where a Vercel maintainer gives the UA `Vercelbot (+https://vercel.com)`; a separate community thread cites a different UA (`vercel-screenshot/1.0`). Six years stale and ambiguous, with no IPs/rDNS/robots token published. Resolve the UA against a Vercel-owned page first. | 2026-07-18 |
